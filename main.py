...
from evaluate_models_utils import CF_evaluate_model_link_prediction
from utils.metrics import get_link_prediction_metrics
from utils.DataLoader import get_idx_data_loader, get_link_prediction_data,load_or_create_data,JointLoss,SimpleCounterfactualLoss
...
if __name__ == "__main__":
    ...
    # test_data=load_or_create_testdata(dataset_name=args.dataset_name, val_ratio=args.val_ratio, test_ratio=args.test_ratio)
    # p=time.time()
    print(f"[DEBUG] train_data长度 = {len(train_data.src_node_ids)}")
    print(f"[DEBUG] test_data长度 = {len(test_data.src_node_ids)}")
    print(f"[DEBUG] new_node_val_data长度 = {len(new_node_val_data.src_node_ids)}")
    print(f"[DEBUG] new_node_test_data长度 = {len(new_node_test_data.src_node_ids)}")
    ...
    for run in range(args.num_runs):
        ...
        #criterion = JointLoss(alpha=0.5, temp=0.1)
        criterion = SimpleCounterfactualLoss(alpha=0.7)
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                batch_src_node_ids, batch_dst_node_ids, batch_node_interact_times, batch_edge_ids, batch_max_ids,batch_max_dst= \
                    ...
                    train_data.node_interact_times[train_data_indices], train_data.edge_ids[train_data_indices],train_data.max_ids_list[train_data_indices],train_data.max_dst_list[train_data_indices]
                ...
                loss, loss_fact, loss_contrast = criterion(predicts, labels, z_pos, z_cf, z_neg)
            ...
            val_losses, val_metrics = CF_evaluate_model_link_prediction(model_name=args.model_name,
                                                                     model=model,
            ...
            new_node_val_losses, new_node_val_metrics = CF_evaluate_model_link_prediction(model_name=args.model_name,
                                                                                       model=model,
                                                                                       neighbor_sampler=full_neighbor_sampler,
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                test_losses, test_metrics = CF_evaluate_model_link_prediction(model_name=args.model_name,
                                                                            model=model,
                                                                            neighbor_sampler=full_neighbor_sampler,
                ...
                new_node_test_losses, new_node_test_metrics = CF_evaluate_model_link_prediction(model_name=args.model_name,
                                                                                                model=model,
                                                                                                neighbor_sampler=full_neighbor_sampler,
