...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, edge_ids: np.ndarray, labels: np.ndarray,
        max_ids_list: np.ndarray, max_dst_list: np.ndarray,max_src_label_list: np.ndarray, max_dst_label_list: np.ndarray):
        """
        ...
        self.max_dst_list=max_dst_list
    ...
    def save_data(self, folder_name: str):
        ...
        if not os.path.exists(folder_path):
            ...
            self.max_dst_list = np.load(os.path.join(folder_path, 'max_dst_list.npy'))
        ...
        np.save(os.path.join(folder_path, 'max_dst_list.npy'), self.max_dst_list)
...
class NeighborSampler: # 邻居节点采样器
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        the returned interactions are sorted by time. ## 提取在目标交互时间之前的目标节点所有的交互，返回按照时间顺序进行排列
    ...
    # def find_neighbors_my(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
    #     """
    #     extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
    #     the returned interactions are sorted by time. ## 提取在目标交互时间之前的目标节点所有的交互，返回按照时间顺序进行排列
    #     :param node_id: int, node id
    ...
    #     # print("#############",self.nodes_neighbor_ids[node_id].shape, self.nodes_edge_ids[node_id].shape, self.nodes_neighbor_times[node_id].shape)
    #     # 这里的node_id可能是 max_id,self.nodes_neighbor_ids有多大呢
    #     #print("##################",node_id)
    #     # print("##################",len(self.nodes_neighbor_ids)) # 9228 9179 这里的数值怎么会变呢
    #     # print("$$$$$$$$$$$$$$$$$$",self.nodes_neighbor_ids[1600]) # 
    #     if return_sampled_probabilities:
    #         return self.nodes_neighbor_ids[node_id], self.nodes_edge_ids[node_id], self.nodes_neighbor_times[node_id], \
    #                self.nodes_neighbor_sampled_probabilities[node_id]
    #     else:
    #         return self.nodes_neighbor_ids[node_id], self.nodes_edge_ids[node_id], self.nodes_neighbor_times[node_id], None

    def find_neighbors_after(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        extracts all the interactions happening before interact_time (less than interact_time) for node_id in the overall interaction graph
        the returned interactions are sorted by time. ## 提取在目标交互时间之后的目标节点所有的交互，返回按照时间顺序进行排列
        :param node_id: int, node id
        :param interact_time: float, interaction time
        :param return_sampled_probabilities: boolean, whether return the sampled probabilities of neighbors
        :return: neighbors, edge_ids, timestamps and sampled_probabilities (if return_sampled_probabilities is True) with shape (historical_nodes_num, )
        ...
        i = np.searchsorted(self.nodes_neighbor_times[node_id],
                            interact_time)  # 在数组a中插入数组v（并不执行插入操作），返回一个下标列表，这个列表指明了v中对应元素应该插入在a中那个位置上
...
class NegativeEdgeSampler(object): # 负样本采样器
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
        ...
        self.unique_src_node_ids = np.unique(src_node_ids)# np.unique()函数 去除其中重复的元素 ，并按元素由小到大返回一个新的无元素重复的元组或者列表。
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        历史抽样策略，首先在当前批中没有的历史边中随机抽样，如果历史边的数量小于size，则用随机抽样的边填充剩余的边
        :param size: int, number of sampled negative edges
        :param batch_src_node_ids: ndarray, shape (batch_size, ), source node ids in the current batch
        :param batch_dst_node_ids: ndarray, shape (batch_size, ), destination node ids in the current batch
        :param current_batch_start_time: float, start time in the current batch
    ...
    def inductive_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        归纳抽样策略，首先在不是观察到的边和当前批的归纳边中随机抽样。如果感应边的数量小于大小，则用随机抽样的边填充剩余的边
        :param size: int, number of sampled negative edges
        :param batch_src_node_ids: ndarray, shape (batch_size, ), source node ids in the current batch
        :param batch_dst_node_ids: ndarray, shape (batch_size, ), destination node ids in the current batch
        :param current_batch_start_time: float, start time in the current batch
